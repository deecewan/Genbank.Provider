...
type internal Testing() =
  ...
  static member MakeSimulatedTypeProviderConfig(resolutionFolder: string, 
                                                runtimeAssembly: string, 
                                                runtimeAssemblyRefs: string list, 
                                                ?isHostedExecution, 
    ...
    let (?<-) cfg prop value =
      let ty = cfg.GetType()
      match ty.GetProperty
              (prop, 
               BindingFlags.Instance ||| BindingFlags.Public 
               ||| BindingFlags.NonPublic) with
      | null -> 
        let fld =
          ty.GetField
            (prop, 
        ...
        if fld = null then 
          failwith
            ("expected TypeProviderConfig to have a property or field " + prop)
    ...
    let dllInfos =
      [ yield DllInfo(runtimeAssembly)
        for r in runtimeAssemblyRefs do
          yield DllInfo(r) ]
    ...
    let systemRuntimeContainsType =
      (fun (_s: string) -> 
      if tcImports.DllInfos.Length = 1 then true
      else true)
    cfg?systemRuntimeContainsType <- systemRuntimeContainsType
    //Diagnostics.Debugger.Launch() |> ignore
    Diagnostics.Debug.Assert(cfg.GetType()
                                .GetField("systemRuntimeContainsType", 
                                          BindingFlags.NonPublic 
                                          ||| BindingFlags.Public 
                                          ||| BindingFlags.Instance)
                             |> isNull
                             |> not)
    Diagnostics.Debug.Assert(systemRuntimeContainsType.GetType()
                                                      .GetField("tcImports", 
                                                                BindingFlags.NonPublic 
                                                                ||| BindingFlags.Public 
                                                                ||| BindingFlags.Instance)
                             |> isNull
                             |> not)
    Diagnostics.Debug.Assert(typeof<TcImports>
                               .GetField("dllInfos", 
                                         BindingFlags.NonPublic 
                                         ||| BindingFlags.Public 
                                         ||| BindingFlags.Instance)
    ...
    Diagnostics.Debug.Assert(typeof<TcImports>
                             ...
                             |> isNull
                             |> not)
    Diagnostics.Debug.Assert(typeof<DllInfo>
  ...
  static member GenerateProvidedTypeInstantiation(resolutionFolder: string, 
                                                  runtimeAssembly: string, 
                                                  runtimeAssemblyRefs: string list, 
                                                  typeProviderForNamespacesConstructor, 
                                                  args: obj []) =
    let cfg =
      Testing.MakeSimulatedTypeProviderConfig
        (resolutionFolder, runtimeAssembly, runtimeAssemblyRefs)
    ...
    let providedType =
      providedTypes
      |> Array.tryFind
           (fun ty -> tp.GetStaticParameters(ty).Length = args.Length)
      |> function 
      | None -> 
        failwithf "couldn't find a type in type provider with %d static args " 
          args.Length
      | Some ty -> ty
    ...
    let providedType =
      ...
      | args -> 
        let typeName =
          if (providedType.Attributes 
              &&& enum(int32 TypeProviderTypeAttributes.IsErased)) <> enum 0 then 
            providedType.Name + (args
                                 |> Seq.map(fun s -> 
                                      ",\"" + (if s = null then ""
                                               else s.ToString())
                                      + "\"")
  ...
  static member FormatProvidedType(tp: ITypeProvider, t: Type, ?signatureOnly, 
                                   ?ignoreOutput, ?maxDepth, ?maxWidth, 
                                   ?useQualifiedNames) =
    ...
    let hasUnitOfMeasure(t: System.Type) =
      t.IsGenericType && (t.Namespace = "System") 
    ...
    let isFSharpUnitAnnotated(t: System.Type) =
      t.IsGenericType 
      && not(t.GetGenericTypeDefinition().IsGenericTypeDefinition)
    let knownNamespaces =
      [ t.Namespace; "Microsoft.FSharp.Core"; "Microsoft.FSharp.Core.Operators"; 
        "Microsoft.FSharp.Collections"; "Microsoft.FSharp.Control"; 
        "Microsoft.FSharp.Text" ] |> Set.ofSeq
    let pending = new Queue<_>()
    let visited = new HashSet<_>()
    
    let add t =
      if visited.Add t then pending.Enqueue t
    ...
    let rec toString useFullName (t: Type) =
      ...
      let innerToString(t: Type) =
        ...
        | t when t.IsArray -> 
          (t.GetElementType() |> toString useFullName) + "[]"
        ...
        | t when t.IsGenericType -> 
          let args =
            if useFullName then 
              ...
              |> Seq.map(if hasUnit then (fun t -> t.Name)
                         else toString useFullName)
            else t.GetGenericArguments() |> Seq.map(fun _ -> "_")
          if t.FullName.StartsWith "System.Tuple`" then String.concat " * " args
            ...
            let name, reverse =
              match t with
              | t when hasUnit -> 
                toString useFullName t.UnderlyingSystemType, false
              // Short names for some known generic types
              | t when not useQualifiedNames 
                       && t.GetGenericTypeDefinition().Name = typeof<int seq>
                            .GetGenericTypeDefinition().Name -> "seq", true
              | t when not useQualifiedNames 
                       && t.GetGenericTypeDefinition().Name = typeof<int list>
                            .GetGenericTypeDefinition().Name -> "list", true
              | t when not useQualifiedNames 
                       && t.GetGenericTypeDefinition().Name = typeof<int option>
                            .GetGenericTypeDefinition().Name -> "option", true
              | t when not useQualifiedNames 
                       && t.GetGenericTypeDefinition().Name = typeof<int ref>
                            .GetGenericTypeDefinition().Name -> "ref", true
              | t when not useQualifiedNames && t.Name = "FSharpAsync`1" -> 
                "async", true
              // Short names for types in F# namespaces
              | t when not useQualifiedNames 
                       && knownNamespaces.Contains t.Namespace -> t.Name, false
              | t -> 
                (if useFullName then fullName t
                 else t.Name), false
            
            let name = name.Split('`').[0]
            if reverse then args + " " + name
            else name + "<" + args + ">"
        // Short names for types in F# namespaces
        | t when not useQualifiedNames && knownNamespaces.Contains t.Namespace -> 
          t.Name
      ...
      let rec warnIfWrongAssembly(t: Type) =
        ...
        | t when t.IsGenericType -> 
          defaultArg (t.GetGenericArguments()
                      |> Seq.map warnIfWrongAssembly
                      |> Seq.tryFind(fun s -> s <> "")) ""
        | t when t.IsArray -> warnIfWrongAssembly <| t.GetElementType()
        | t -> 
          if not t.IsGenericParameter 
             && t.Assembly = Assembly.GetExecutingAssembly() then 
            " [DESIGNTIME]"
          else ""
      ...
      if ignoreOutput then ""
        ...
        (toString useFullName t.DeclaringType) + "+" + (innerToString t) 
    ...
    let printExpr expr =
      ...
      let getCurrentIndent() =
        ...
        if lastEnterPos = -1 then sb.Length + 4
        else sb.Length - lastEnterPos - 1
      ...
      let inline getAttrs attrName m =
        ...
        |> Seq.filter
             (fun attr -> attr.Constructor.DeclaringType.Name = attrName)
      ...
      and printCall fromPipe printName (mi: MethodInfo) args =
        ...
        if fromPipe && List.length args = 1 then printName()
          ...
          | arg :: args -> 
            ...
            for arg in args do
              print ", "
              if isBigExpression arg then breakLine indent
              printExpr false true arg
            print ")"
        else 
          print "("
      ...
      and printExpr fromPipe needsParens =
        ...
        | Call(instance, mi, args) -> 
          if mi.Name = "GetArray" 
             && mi.DeclaringType.FullName = "Microsoft.FSharp.Core.LanguagePrimitives+IntrinsicFunctions" then 
            printExpr false true args.Head
            print ".["
            printExpr false true args.Tail.Head
            print "]"
          elif mi.DeclaringType.IsGenericType 
               && mi.DeclaringType.GetGenericTypeDefinition().Name = typeof<int option>
                    .GetGenericTypeDefinition().Name then 
          ...
          elif mi.Attributes &&& MethodAttributes.SpecialName = MethodAttributes.SpecialName 
               && mi.Name.StartsWith "get_" && args.IsEmpty then 
            // property get
            match instance with
        ...
        | Let(var, value, body) -> 
          ...
          let body =
            match body with
            | TryFinally(tryExpr, finallyExpr) when finallyExpr.ToString()
                                                               .Replace("\n", 
                                                                        null)
                                                               .Replace(" ", 
                                                                        null) = usePattern -> 
              ...
              tryExpr
            | _ -> 
              if var.IsMutable then bprintf sb "let mutable %s = " var.Name
              else bprintf sb "let %s = " var.Name
              body
          match value with
          | Let _ -> 
            breakLine(indent + 4)
        ...
        | Value(value, typ) when typ = typeof<string> 
        ...
        | NewObject(ci, args) -> 
          let getSourceConstructFlags(attr: CustomAttributeData) =
            let arg =
              ...
              |> Seq.filter
                   (fun arg -> arg.ArgumentType.Name = "SourceConstructFlags")
              |> Seq.head
            arg.Value :?> int
          
          let compilationMappings =
            getAttrs "CompilationMappingAttribute" ci.DeclaringType
          if not(Seq.isEmpty compilationMappings) 
             && (getSourceConstructFlags(Seq.head compilationMappings)) = int 
                                                                            SourceConstructFlags.RecordType then 
            print "{ "
            let indent = getCurrentIndent()
            let recordFields = FSharpType.GetRecordFields(ci.DeclaringType)
            args
            |> List.iteri(fun i arg -> 
                 if i > 0 then breakLine indent
        ...
        | NewDelegate(typ, vars, expr) -> 
          ...
          match expr with
          | Var v when not vars.IsEmpty && vars.Tail.IsEmpty && vars.Head = v -> 
            print "(id)"
          | _ -> 
            let indent = getCurrentIndent()
            if vars.IsEmpty then print "(fun () -> "
            else 
              print "(fun"
              for var in vars do
            ...
            else printExpr false false expr
          print ")"
        | NewTuple(exprs) -> 
        ...
        | TupleGet(expr, index) -> 
          ...
          let rec getTupleLength(typ: Type) =
            ...
            if length = 0 then // happens in the Apiary provider
              ...
              typeNameSuffix.Substring(0, typeNameSuffix.IndexOf('[')) 
              |> Int32.Parse
            else 
          ...
          for i in 0..tupleLength - 1 do
            if i = index then print varName
            else print "_"
            if i <> tupleLength - 1 then print ","
          print " = "
          printExpr false false expr
          print(" in " + varName + ")")
    ...
    let print(str: string) =
      if not ignoreOutput then sb.Append(str) |> ignore
    
    let println() =
      if not ignoreOutput then sb.AppendLine() |> ignore
    ...
    let printMember(memberInfo: MemberInfo) =
      ...
      let rec eraseType(typ: Type): Type =
        match typ with
        | t when t.Attributes 
                 &&& enum(int32 TypeProviderTypeAttributes.IsErased) <> enum 0 -> 
          eraseType typ.BaseType
        ...
        | t when t.IsArray -> 
          ...
          if rank = 0 then et.MakeArrayType()
          else et.MakeArrayType(rank)
        ...
        let vs =
          [ if not m.IsStatic then yield ("this", eraseType m.DeclaringType)
            for p in m.GetParameters() do
              yield (p.Name, eraseType p.ParameterType) ]
      ...
      let getName(m: MemberInfo) =
        if memberInfo.Name.Contains(" ") then "``" + m.Name + "``"
        else m.Name
      ...
      | (:? ConstructorInfo as cons) when cons.GetType().Name = "ProvidedConstructor" -> 
        if not ignoreOutput then 
          print 
          <| "new : " + (toSignature <| cons.GetParameters()) + " -> " 
      ...
      | (:? FieldInfo as field) when field.GetType().Name = "ProvidedField" -> 
        ...
        if not ignoreOutput then 
          print 
          <| "val " + (getName field) + ": " + (toString true field.FieldType) 
             + value
      | (:? PropertyInfo as prop) when prop.GetType().Name = "ProvidedProperty" -> 
        if not ignoreOutput then 
          print 
          <| (if (prop.CanRead && prop.GetGetMethod().IsStatic 
                  || prop.CanWrite && prop.GetSetMethod().IsStatic) then 
                "static "
              else "") + "member " + (getName prop) + ": " 
             + (toString true prop.PropertyType) + " with " + (if prop.CanRead 
                                                                  && prop.CanWrite then 
                                                                 "get, set"
                                                               else if prop.CanRead then 
                                                                 "get"
                                                               else "set")
        if not signatureOnly then 
          if prop.CanRead then getMethodBody(prop.GetGetMethod()) |> printExpr
          if prop.CanWrite then getMethodBody(prop.GetSetMethod()) |> printExpr
      | (:? MethodInfo as m) when m.GetType().Name = "ProvidedMethod" -> 
        if m.Attributes &&& MethodAttributes.SpecialName 
           <> MethodAttributes.SpecialName then 
          if not ignoreOutput then 
            print 
            <| (if m.IsStatic then "static "
                else "") + "member " + (getName m) + ": " 
    ...
    while pending.Count <> 0 && !currentDepth <= maxDepth do
      ...
      for t in pendingForThisDepth do
        ...
        let bt =
          if isNull t.BaseType then typeof<obj>
          else t.BaseType
        print " : "
...
module internal Targets =
  ...
  let installedReferenceAssembliesRootOnWindows() =
    Environment.GetFolderPath Environment.SpecialFolder.ProgramFilesX86 
    ++ "Reference Assemblies" ++ "Microsoft"
  
  let fsharpInstalledAssembliesRoot() =
    if runningOnWindows then 
      installedReferenceAssembliesRootOnWindows() ++ "FSharp"
    else monoRoot() ++ "fsharp" ++ "api"
  
  let installedPortableAssembliesRoot() =
    (if runningOnWindows then 
       installedReferenceAssembliesRootOnWindows() ++ "Framework"
     else monoRoot() ++ "xbuild-frameworks")
    ++ ".NETPortable"
  
  let installedNet45AssembliesRoot() =
    if runningOnWindows then 
      installedReferenceAssembliesRootOnWindows() ++ "Framework" 
      ++ ".NETFramework" ++ "v4.5"
    else monoRoot() ++ "4.5-api"
  ...
  let packagesDirectory() =
    ...
    let rec loop dir =
      ...
      if Directory.Exists(dir ++ "packages") then dir ++ "packages"
      else 
        let parent = Path.GetDirectoryName(dir)
        match parent with
        | null | "" -> 
          failwith("couldn't find packages directory anywhere above  " + root)
        | _ -> loop parent
    loop root
  
  let paketPackagesGroupDirectory paketGroup =
    let pd = packagesDirectory()
    if Directory.Exists(pd ++ paketGroup) then Some(pd ++ paketGroup)
    else None
  
  let private fsharpCoreFromInstalledAssemblies fsharp profile =
    match fsharp, profile with
    | "3.1", "net45" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETFramework" ++ "v4.0" ++ "4.3.1.0" 
      ++ "FSharp.Core.dll" |> Some
    | "4.0", "net45" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETFramework" ++ "v4.0" ++ "4.4.0.0" 
      ++ "FSharp.Core.dll" |> Some
    | "4.1", "net45" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETFramework" ++ "v4.0" ++ "4.4.1.0" 
      ++ "FSharp.Core.dll" |> Some
    | "3.1", "portable47" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETPortable" ++ "2.3.5.1" 
      ++ "FSharp.Core.dll" |> Some
    | "3.1", "portable7" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.3.1.0" 
      ++ "FSharp.Core.dll" |> Some
    | "3.1", "portable78" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.78.3.1" 
      ++ "FSharp.Core.dll" |> Some
    | "3.1", "portable259" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.259.3.1" 
      ++ "FSharp.Core.dll" |> Some
    | "4.0", "portable47" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETPortable" ++ "3.47.4.0" 
      ++ "FSharp.Core.dll" |> Some
    | "4.0", "portable7" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.7.4.0" 
      ++ "FSharp.Core.dll" |> Some
    | "4.0", "portable78" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.78.4.0" 
      ++ "FSharp.Core.dll" |> Some
    | "4.0", "portable259" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.259.4.0" 
      ++ "FSharp.Core.dll" |> Some
    | "4.1", "portable47" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETPortable" ++ "3.47.4.1" 
      ++ "FSharp.Core.dll" |> Some
    | "4.1", "portable7" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.7.4.1" 
      ++ "FSharp.Core.dll" |> Some
    | "4.1", "portable78" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.78.4.1" 
      ++ "FSharp.Core.dll" |> Some
    | "4.1", "portable259" -> 
      fsharpInstalledAssembliesRoot() ++ ".NETCore" ++ "3.259.4.1" 
      ++ "FSharp.Core.dll" |> Some
    ...
    | _ -> 
      failwith
        (sprintf "unimplemented  profile, fsharpVersion = %s, profile = %s" 
           fsharp profile)
  
  let paketPackageFromMainPaketGroup packageName =
    let pd = packagesDirectory()
    if Directory.Exists(pd ++ packageName) then pd ++ packageName
    else 
      failwithf 
        "couldn't find %s/NETStandard.Library, whcih is needed for testing .NET Standard 2.0 code generation of a type provider using these utilities" 
        pd
  ...
  let private fsharpRestoredAssembliesPath fsharp profile =
    ...
    let compatProfiles =
      ...
      | "netcoreapp2.0" -> 
        [ "netcoreapp2.0"; "netstandard2.0"; "netstandard1.6" ]
    ...
    let groupDirectory =
      ...
      | None -> 
        printfn 
          "couldn't find paket packages group %s.  Assuming %s/FSharp.Core is for F# version %s" 
          paketGroup (packagesDirectory()) fsharp
        packagesDirectory()
      | Some dir -> dir
    
    compatProfiles
    |> List.tryPick(fun profileFolder -> 
         let file =
           groupDirectory ++ "FSharp.Core" ++ "lib" ++ profileFolder 
           ++ "FSharp.Core.dll"
  ...
  let sysAssembliesPath profile =
    ...
    | "portable47" -> 
      installedPortableAssembliesRoot() ++ "v4.0" ++ "Profile" ++ "Profile47"
    | "portable7" -> 
      installedPortableAssembliesRoot() ++ "v4.5" ++ "Profile" ++ "Profile7"
    | "portable78" -> 
      installedPortableAssembliesRoot() ++ "v4.5" ++ "Profile" ++ "Profile78"
    | "portable259" -> 
      installedPortableAssembliesRoot() ++ "v4.5" ++ "Profile" ++ "Profile259"
  ...
  let FSharpCoreRef fsharp profile =
    ...
    | _ -> 
      ...
      | _ -> 
        ...
        | Some path -> 
          failwith
            ("couldn't find FSharp.Core.dll at either '" + path + "' or '" 
             + restoredFSharpCore + "'")
        | None -> 
          failwith
            ("couldn't find FSharp.Core.dll at '" + restoredFSharpCore + "'")
  
  let FSharpRefs fsharp profile =
    [ match profile with
      | "portable7" | "portable78" | "portable259" -> 
        let sysPath = sysAssembliesPath profile
        for asm in [ "System.Runtime"; "mscorlib"; "System.Collections"; 
                     "System.Core"; "System"; "System.Globalization"; 
                     "System.IO"; "System.Linq"; "System.Linq.Expressions"; 
                     "System.Linq.Queryable"; "System.Net"; 
                     "System.Net.NetworkInformation"; "System.Net.Primitives"; 
                     "System.Net.Requests"; "System.ObjectModel"; 
                     "System.Reflection"; "System.Reflection.Extensions"; 
                     "System.Reflection.Primitives"; 
                     "System.Resources.ResourceManager"; 
                     "System.Runtime.Extensions"; 
                     "System.Runtime.InteropServices.WindowsRuntime"; 
                     "System.Runtime.Serialization"; "System.Threading"; 
                     "System.Threading.Tasks"; "System.Xml"; "System.Xml.Linq"; 
                     "System.Xml.XDocument"; "System.Runtime.Serialization.Json"; 
                     "System.Runtime.Serialization.Primitives"; "System.Windows" ] do
  ...
  let supportsFSharp31() =
    (try 
       File.Exists(FSharpCore31Ref())
     with _ -> false)
  
  let supportsFSharp40() =
    (try 
       File.Exists(FSharpCore40Ref())
     with _ -> false)
  
  let supportsFSharp41() =
    (try 
       File.Exists(FSharpCore41Ref())
     with _ -> false)
  
  let hasPortable47Assemblies() =
    Directory.Exists(sysAssembliesPath "portable47")
  let hasPortable7Assemblies() = Directory.Exists(sysAssembliesPath "portable7")
  let hasPortable78Assemblies() =
    Directory.Exists(sysAssembliesPath "portable78")
  let hasPortable259Assemblies() =
    Directory.Exists(sysAssembliesPath "portable259")
